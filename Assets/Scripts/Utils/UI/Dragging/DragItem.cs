...
namespace RPG.Core.UI.Dragging
{
        ...
        where T : class
    {
        ...
        private void DropItemIntoContainer(IDragDestination<T> destination)
        {
            ...
            if(commandSplit == true)
            {
                if(destination.MaxAcceptable(source.GetItem()) < 1)
                {
                    return;
                }

                SplitItems(destinationContainer, sourceContainer);
                return;
            }
        ...
        private void SplitItems(IDragContainer<T> destination, IDragContainer<T> source)
        {
            var removedSourceNumber = source.GetNumber();
            var removedSourceItem = source.GetItem();
            
            
            var numberToSplit = removedSourceNumber;
            int halftoCheck = removedSourceNumber/2;
            int otherHalftoCheck = removedSourceNumber - halftoCheck;
            int half = removedSourceNumber/2;
            int otherHalf = removedSourceNumber - half;

            if(half < 1 || otherHalf < 1)
            {
                return;
            }
            ...
            if (!EventSystem.current.IsPointerOverGameObject())
            {
                var destinationContainer = destination as IDragDestination<T>;
                destinationContainer = parentCanvas.GetComponent<IDragDestination<T>>();

                source.RemoveItems(removedSourceNumber);

                source.AddItems(removedSourceItem, half);
                destinationContainer.AddItems(removedSourceItem, otherHalf);
                return;
            }
            
            source.RemoveItems(removedSourceNumber);
            source.AddItems(removedSourceItem, half);
            destination.AddItems(removedSourceItem, otherHalf);
        ...
        private bool AttemptSimpleTransfer(IDragDestination<T> destination)
        {
            ...
            var removeSource = source as IDragContainer<T>;
            

            var acceptable = destination.MaxAcceptable(draggingItem);
            var toTransfer = Mathf.Min(acceptable, draggingNumber);
            var toSplit = draggingNumber / 2;
